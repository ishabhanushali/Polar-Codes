...
def decodeSCL(r,i,level,k,frozen):
        #1st is path-metric,2nd - codeword-x,3rd - message in good channels
        if level == 0:
            decoded_x = []
            if frozen[mt.floor(i)] == 1:#If Frozen
                if r[0] >= 0:#
        ...
        L1 = r[0:l]
        L2 = r[l:len(r)]
        #Generate Belief for stepLeft
        f = F(L1,L2)
        list = []
        #Do stepL-Go leftChild and get decoded_values
        left_x = decodeSCL(f,i,level-1,k,frozen)
        for j in range (0,len(left_x)):
            #Generate beliefs for stepRight
            g = G(L1,L2,left_x[j][1])
            #Do stepR - Go to rightChild and get list of decoded_values
            right_x = decodeSCL(g,i+2**(level-1),level-1,k,frozen)
            #For each decoded_value on right side,encode it with corredponding left side and stor it in list
            #Encoding here means 1.adding pathmetric from left and right side
            #2.Finding codeword at that level
            #3.Getting u1 to uk at that level
            for m in range (0,len(right_x)):
                lx = encode(left_x[j],right_x[m])
                list.append(lx)
        #Sort the list in ascending order on basis on path metric
        list = sorted(list, key=lambda x: x[0])
        #Store and return only the first k values(list size)
        if len(list) > k:
            list = list[0:k]
        return list
...
for k in k_l:
    ...
    plt.plot(ebs,Psuccess,label = 'messageSize(k) = ' + str(k))
    plt.title('Decoding Probability with Change in Eb/N0(Noise) For n = ' + str(n) + ' for different k values')
